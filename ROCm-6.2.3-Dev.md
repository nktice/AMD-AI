...
2025-01-26 - Various updates after some time away... 

2025-10-19 - This page has been made for historical uses, and depicated... new versions are out, but as old details may help, I'm saving this here. 
...
# In the following - when I tried with the same date, I got an error, so these dates mismatch,
# if you have issues, odds are that this has resolved, and you'll need matching dates. 
export TORCH_COMMAND="pip install --pre torch==2.7.0.dev20250220+rocm6.3  torchvision==0.22.0.dev20250221+rocm6.3 --extra-index-url https://download.pytorch.org/whl/nightly/rocm6.3"
...
python3.10 -m pip install --pre torch==2.7.0.dev20250220+rocm6.3  torchvision==0.22.0.dev20250221+rocm6.3  torchsde torchaudio einops transformers>=4.25.1 safetensors>=0.4.2 aiohttp pyyaml Pillow scipy tqdm psutil  --extra-index-url https://download.pytorch.org/whl/nightly/rocm6.3
...
pip install --pre -U torch==2.7.0.dev20250221+rocm6.3  torchvision==0.22.0.dev20250221+rocm6.3 \
...
```bash
pip install -r requirements_amd.txt  --extra-index-url https://download.pytorch.org/whl/nightly/rocm6.3
```
